package com.github.base.security.keycloak.filter;
 ...
 * Explanation: When the browser makes an AJAX request a special header field is set the
 * X-Requested-With. This field is a non-standard header field, but it is used in many JavaScript
 * libraries. On the server side in this XRequestedHeaderFilter class, we validate the request to
 * make sure the header exists in the request. If it does not, the request is rejected. This works
 * because of the support of the Same-origin Policy (SOP) To add this special header to a request
 * within the context of the browser, the attacker needs to use XMLHttpRequest. But because of SOP,
 * you can not by default send an AJAX request to a third-party domain using XMLHttpRequest.
 */
public class XRequestedHeaderFilter extends GenericFilterBean {
  ...
  public XRequestedHeaderFilter(HttpProperties properties) {
  ...
  protected void doFilter(HttpServletRequest request, HttpServletResponse response,
    ...
    Cookie cookie = Arrays.stream(cookies).filter(c -> c.getName().equals(cookieName)).findFirst()
        .orElse(null);
    if (cookie == null) {
